...
module.exports = function() {
  ...
  var replaceMapStyle = curry(function(tolkenKey, tolkenValue, str) {
    return compose(
      ...
      map(compose(
        function(value) {
          return value[0].replace(new RegExp(removeDot(tolkenKey), 'g'), tolkenValue) +
                  ':' + value[1];
        },
        split(/\:/))),
      split(/,/),
  ...
  var replaceAngularClass = function(ngClass) {
    return replacer(function(tolkenKey, tolkensValues) {
      return {
        ...
        process: function(index, attrValue) {
          return compose(
            ifElse(isStringStyle,
              replaceStringStyle(tolkenKey, tolkensValues[index]),
              identity),
            ifElse(isMapStyle,
              replaceMapStyle(tolkenKey, tolkensValues[index]),
              identity))(attrValue);
        }
      };
    });
